...
def format_results_to_table(results):
    ...
    if isinstance(results, str):
        # Always try to parse as a table and display with headers and values
        try:
            # Try to read as markdown table
            if results.strip().startswith("|"):
                df = pd.read_csv(io.StringIO(results), sep="|", engine="python").dropna(axis=1, how="all")
                return df.to_html(classes="result-table", index=False)
            # Try to read as grid table (tabulate output)
            elif "+" in results and "|" in results:
                # Extract lines between the first and last '+'
                ...
                if table_lines:
                    # Convert grid table to DataFrame
                    import re
                    header_line = None
                    for i, line in enumerate(table_lines):
                        if re.match(r"\|[\s\w\-\_\d]+\|", line):
                            header_line = i
                            break
                    if header_line is not None:
                        headers = [h.strip() for h in table_lines[header_line].split("|") if h.strip()]
                        data_rows = []
                        for row in table_lines[header_line+2:]:
                            if row.startswith("+"): break
                            data = [cell.strip() for cell in row.split("|") if cell.strip()]
                            if data: data_rows.append(data)
                        if data_rows:
                            df = pd.DataFrame(data_rows, columns=headers)  # type: ignore
                            return df.to_html(classes="result-table", index=False)
            # Fallback: show as preformatted text
            return f'<pre class="result-table">{results}</pre>'
        except Exception:
            return f'<pre class="result-table">{results}</pre>'
